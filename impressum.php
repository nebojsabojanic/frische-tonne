<?php 
    include "inc/config.php";

    $pageTitle = "Impressum";
    $pageTitleShort = "impressum";

?>

<?php include "inc/header-alt.php"; ?>


    <main class="content content--alt grid-1280">

        <div class="title">
            <span class="title__subtitle">Impressum</span>
            <h2 class="title--blue">Frische Tonne GmbH</h2>
        </div>

        <div class="law">

            <div class="law__content">

                <address>
                    <p>Im Maisel 3</p>

                    <p>65232 Taunusstein</p>

                    <p>(Deutschland)</p>
                </address>
                    
                <p class="paragraph">Telefon: <a href="tel:004961289239215">06128 - 9239215</a></p>
                <p class="paragraph">Telefax: 06128 - 982556</p>
                <p class="paragraph">e-Mail: <a href="mailto:info@frische-tonne.de">info@frische-tonne.de</a></p>
 
 
                <p class="paragraph">Steuernummer: DE327850882</p>
                <p class="paragraph">Gläubiger ID DE 04 Z 1 Z 0000721480</p>
                <p class="paragraph">Erfüllungsort und Gerichtsstand: Wiesbaden</p>

                <p class="paragraph">Geschäftsführer: Melanie Heun-Sass</p>
            
        </div>

    </main>


<?php include "inc/footer.php"; ?>
